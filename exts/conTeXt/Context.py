...
class Context:
    ...
    async def get_cmds(self, CH=None):
        handlers = CH if CH else self.bot.handlers
        cmds = {}
        for CH in handlers:
    ...
    async def send(self, destination, message=None, embed=None, file_data=None, file_name=None, split=False, code=False, everyone=False):
        """
        ...
        if (not file_data) and (not file_name) and (message is None) and (embed is None):
        ...
        if self.bot_err[0] != 0:
            ...
            return None
        if file_data and embed:
            ch_id, guild_id = await self.bot._resolve_destination(destination)
            message = str(message) if message is not None else None
            embed = embed.to_dict()
            data = await self.bot.http.send_file(ch_id, file_data, guild_id=guild_id,
                                                  filename=file_name if file_name else "File", content=message, tts=False, embed=embed)
            channel = self.bot.get_channel(data.get('channel_id'))
            message = self.bot.connection._create_message(channel=channel, **data)
            self.sent_messages.append(message)
            return message
        elif file_data:
            msg = await self.bot.send_file(destination, fp=file_data, filename=file_name if file_name else "File", content=message)
            self.sent_messages.append(msg)
            return msg
        elif file_name:
            msg = await self.bot.send_file(destination, file_name, content=message)
            self.sent_messages.append(msg)
            return msg
        ...
        if message:
            ...
            if split and (not embed):
                ...
                self.sent_messages.extend(out)
                return out
            else:
                if len(message) >= 2000:
                    self.bot_err = (3, "Tried to send a message which was too long")
                    return
                msg = await self.bot.send_message(destination, str(message), embed=embed)
                self.sent_messages.append(msg)
                return msg
        elif embed:
            msg = await self.bot.send_message(destination, embed=embed)
            self.sent_messages.append(msg)
            return msg
...
class MessageContext(Context):
    def __init__(self, **kwargs):
        ...
        if "message" in kwargs:
            ...
            self.me = self.server.me if self.server else None
            ...
            if dm:
                ...
                if not perms.send_messages:
                    try:
                        ...
                        return await self.reply(message, embed, file_name, dm=True, **kwargs)
...
class CommandContext(MessageContext):
    def __init__(self, **kwargs):
        ...
        if self.used_prefix:
            without_prefix = self.cntnt[len(self.used_prefix):].strip().split()
            self.used_cmd_name = without_prefix[0]
        if self.params is None:
            if self.arg_str is None:
                ...
                if self.used_prefix:
                    ...
                    self.arg_str = self.cntnt[len(self.cntnt.split()[0]):]
