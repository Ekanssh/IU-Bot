    ...
    async def dailies(self, ctx):
        ...
        if c.fetchone():                                                  #if found
            ...
            if secondsRemaining <= 0:                              
                currentDaily += 200
                c.execute("UPDATE Dailies SET dailiesCount = ?, secToReset = '86400' WHERE id = ?", (str(currentDaily), ctx.message.author.id))
            ...
            c.execute("INSERT INTO Dailies VALUES (?, ?, ?)", (ctx.message.author.id, "200", "86400"))
            await ctx.send("You got your 200 dialies! :moneybag:\n You have ₹{}".format(currentDaily))
